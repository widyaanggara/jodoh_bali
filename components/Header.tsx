...
export default function Header() {
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            if (!document.querySelector('header')?.contains(event.target as Node)) {
    ...
    const navLinks = [
        ...
        {
            name: 'Petunjuk Otonan',
            href: '#',
            children: [
                { name: 'Mencari Otonan', href: '/mencari-otonan' },
                { name: 'Ramalan Otonan', href: '/ramalan-otonan' },
            ]
        },
        { name: 'Samyoga', href: '/pawongan-sinergi' },
        ...
        <header className="sticky top-0 z-50 bg-background-light/80 backdrop-blur-md border-b border-accent-gold/10">
            <nav className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center space-x-8 text-sm font-medium">
                    {navLinks.map((link) => (
                        link.children ? (
                            <div key={link.name} className="relative group" onMouseLeave={() => setActiveDropdown(null)}>
                                <button
                                ...
                                >
                                    {link.name}
                                    <span className={`material-icons-outlined text-sm transition-transform duration-300 ${activeDropdown === link.name ? 'rotate-180' : ''}`}>
                                        expand_more
                                    </span>
                                </button>
                    ...
                    {navLinks.map((link) => (
                        link.children ? (
                            <div key={link.name} className="flex flex-col bg-stone-50 rounded-xl overflow-hidden transition-all duration-300">
                                ...
                                <div
                                    className={`flex flex-col border-stone-200 transition-all duration-300 w-full overflow-hidden ${activeMobileDropdown === link.name ? 'max-h-48 border-t opacity-100' : 'max-h-0 border-t-0 opacity-0'
                                        }`}
                                >
                                    {link.children.map(child => (
                                        <Link
